...
function makeBr(value) {
    if (value.match(/^[a-zA-Zа-яА-Я]<sub>\d*<\/sub>$/).length > 1) return "(" + value + ")";
    return value;
}

function getInfo(parsed) {
    var result = {
        vars: parsed.vars.slice(),
        var_values: [],
        vector: [],
        vector_d: {},
        func: parsed.func,
        ...
        is_constant: false,
        constant: -1,
        n: parsed.vars.length,
        total: 1 << parsed.vars.length,
        polinom_values: [],
        sdnf: [],
        sdnf_sets: [],
        sknf: [],
    ...
    for (var i = 0; i < result.total; i++) {
        ...
        var s = [];
        for (var j = 0; j < result.n; j++) s.push(getKD(result.vars[j], var_values[j], f));
        if (f == 1) {
...
function getInfoFromVec(vector) {
    ...
    for (var i = 0; i < result.n; i++) result.vars.push("x<sub>" + (i + 1) + "</sub>");
...
function getPolinom(info) {
    ...
    while (w < info.total) {
        ...
        while (index < info.total) {
            ...
            for (var i = 0; i < w; i++) {
                ...
                solve[k][index] = {
                    v: "<span class='xor'>" + XOR + " " + old[index - w] + "</span>",
                    c: "red"
                };
                solve[k + 1][index] = {
                    v: vector[index],
    ...
    table += "<tr>";
    for (var i = 0; i < info.n; i++) table += "<td>" + info.vars[i] + "</td>";
    table += "<td>" + info.func + "</td>";
    table += "</tr>";
    for (var i = 0; i < info.total; i++) {
        table += "<tr>";
        for (var j = 0; j < info.n; j++) table += "<td>" + info.var_values[i][j] + "</td>";
        table += "<td class='" + solve[0][i].c + "'>" + info.vector[i] + "</td>";
        for (var j = 0; j < 2 * info.n; j++) table += "<td class='" + solve[j + 1][i].c + "'>" + solve[j][i].v + "</td>";
        table += "<td>" + solve[2 * info.n][i] + "</td>";
        table += "</tr>";
    }
    table += "</table>";
    return {
        polinom: polinom.join("<span class='xor'>" + XOR + "</span>"),
        solve: "<div class='scroll-block'>" + table + "</div>"
    };
}

function GetPolinomTriangle(info) {
    var triangle = [];
    for (var i = 0; i < info.total + 2; i++) triangle[i] = [];
    for (var j = 0; j < info.total; j++) {
        triangle[0][j] = info.var_values[j].join("");
        triangle[1][j] = info.polinom_values[j].join("");
    }
    for (var i = 0; i < info.total; i++) triangle[i + 2][0] = info.vector[i];
    for (var j = 1; j < info.total; j++)
        for (var i = 0; i < info.total - j; i++) triangle[i + 2][j] = triangle[i + 2][j - 1] ^ triangle[i + 3][j - 1];
    var table = "<div class='scroll-block'><table class='table'><tr>";
    for (var j = 0; j < info.total; j++) table += "<td>" + triangle[0][j] + "</td>";
    table += "</tr><tr>";
    for (var j = 0; j < info.total; j++) {
        if (triangle[2][j] == 1) {
    ...
    for (var i = 2; i < triangle.length; i++) {
        ...
        for (var j = 1; j < triangle[i].length; j++) table += "<td>" + triangle[i][j] + "</td>";
        table += "</tr>";
    }
    table += "</table></div><br>";
...
function GetPolinomCoefficients(info) {
    ...
    for (var i = 1; i < info.total; i++) solve += " " + XOR + " a<sub>" + func[i].coefficient.join("") + "</sub>" + func[i].vars;
    solve += "</div><br>";
    var coefficients = [];
    ...
    var polinom = [];
    if (coefficients[0] == 1) polinom.push(func[0].vars);
    for (var i = 1; i < info.total; i++) {
        ...
        for (var j = 1; j < coef.length; j++) solve += " " + XOR + " a<sub>" + coef[j].join("") + "</sub>";
        ...
        if (coefficients[i] == 1) polinom.push(func[i].vars);
...
function getPolinomTerms(info) {
    if (info.is_constant && info.constant == 0) return ["0"];
    var vector = info.vector.slice();
    var w = 1;
    while (w < info.total) {
        var old = vector.slice();
    ...
    var polinom = [];
    for (var i = 0; i < info.total; i++)
        if (vector[i] == 1) polinom.push(info.polinom_values[i].join(""));
    return polinom;
}
...
function GetBigTable(info) {
    ...
    table += "<tr>";
    for (var i = 0; i < info.n; i++) table += "<td>" + info.vars[i] + "</td>";
    for (var i = 0; i < subexpr.length; i++) table += "<td class='gray'>" + RPNtoString(subexpr[i]) + "</td>";
    table += "<td class='green'>" + RPNtoString(info.rpn) + "</td></tr>";
    for (var i = 0; i < info.total; i++) {
        table += "<tr>";
        for (var j = 0; j < info.n; j++) table += "<td>" + info.var_values[i][j] + "</td>";
        for (var j = 0; j < subexpr.length; j++) table += "<td class='gray'>" + Calculate(info.vars, info.var_values[i], subexpr[j]) + "</td>";
...
function GetClassification(info, polinom) {
    ...
    for (var i = 0; i < polinom.length && L; i++) {
        if (!IsVariable(polinom[i]) && polinom[i] != "1" && polinom[i] != "0") L = false;
    }
    if (L) {
...
function GetKarnoMap(info, need) {
    ...
    for (var i = 0; i < total1; i++) {
        ...
        for (var j = 0; j < total2; j++) {
            ...
            K[i][j] = [];
            for (var k = 0; k < info.n; k++) K[i][j].push(getKD(info.vars[k], set[k], need));
            map += "<td>" + f + "</td>";
            table[i + 1].push(f);
            karno[i].push(f);
        }
...
function ShowAreaOnKard(table, area) {
    ...
    for (var j = 0; j < table[0].length; j++) html += "<td>" + table[0][j] + "</td>";
    html += "</tr>";
    for (var i = 1; i < table.length; i++) {
        ...
        for (var j = 1; j < table[i].length; j++) {
            ...
            var y = j - 1;
            var index = 0;
            while (index < area.length && !(area[index].x == x && area[index].y == y)) index++;
...
function CheckAreas(area1, area2, area) {
    for (var i = 0; i < area.c.length; i++) {
        ...
        var y = area.c[i].y;
        var i1 = 0;
        while (i1 < area1.c.length && !((area1.c[i1].x == x) && (area1.c[i1].y === y))) i1++;
        i2 = 0;
        while (i2 < area2.c.length && !((area2.c[i2].x == x) && (area2.c[i2].y === y))) i2++;
...
function FindFictives(info) {
    ...
    if (info.is_constant) {
        return {
            html: html,
            solve: "Функция является константой (не зависит от переменных), поэтому все переменные являются фиктивными."
        };
    } else {
        return {
            html: html,
...
$(document).ready(function() {
    karnos = [];
    $.getJSON("/js/json/karno_1.json?v=3", function(res) {
        karnos[0] = res;
        console.log("OK 1");
    });
    $.getJSON("/js/json/karno_2.json?v=3", function(res) {
        karnos[1] = res;
        console.log("OK 2");
    });
    $.getJSON("/js/json/karno_3.json?v=3", function(res) {
        karnos[2] = res;
        console.log("OK 3");
    });
    $.getJSON("/js/json/karno_4.json?v=3", function(res) {
        karnos[3] = res;
        console.log("OK 4");
    });
    $.getJSON("/js/json/karno_5.json?v=3", function(res) {
        karnos[4] = res;
        console.log("OK 5");
    });
    $.getJSON("/js/json/karno_6.json?v=3", function(res) {
        karnos[5] = res;
        console.log("OK 6");
    });
    $.getJSON("/js/json/karno_7.json?v=3", function(res) {
        karnos[6] = res;
        console.log("OK 7");
    });
    $.getJSON("/js/json/karno_8.json?v=3", function(res) {
        karnos[7] = res;
        console.log("OK 8");
    });
    $.getJSON("/js/json/karno_9.json?v=4", function(res) {
        karnos[8] = res;
        console.log("OK 9");
    });
    updateScrollblocks();
    ...
    $("#keyboard-key").click(function() {
        ...
        var t = $("#open-key-text");
        if (t.text() == "Скрыть клавиатуру") {
            t.text("Показать клавиатуру");
        } else {
            t.text("Скрыть клавиатуру");
        }
    });
    $("#settings-key").click(function() {
        $("#settings").slideToggle('normal');
        var t = $("#settings-key-text");
        if (t.text() == "Скрыть настройки") {
            t.text("Показать настройки");
        } else {
            t.text("Скрыть настройки");
        }
    });
    $(".calc-btn").click(function() {
        $(".loader").show();
        setTimeout(function() {
            var text = ($("#function").val() == "" ? $("#function").attr("placeholder") : $("#function").val()).replace(/\s/g, "");
            ...
            try {
                ...
                if ($("#table").is(":checked")) {
                    options += "table ";
                    if (vec == null) html += "<div class='scroll-block'><b>Вектор функция:</b> " + info.vector.join("") + "</div>";
                ...
                $.ajax({
                    url: "https://programforyou.ru/statistics/getStat.shtml?" + "file=calculators//boolean_functions.txt" + "&clicks=" + $("#stat-value").text() + "&function=" + text.replace(/⊕/g, "^").replace(/→/g, "->").replace(/&/g, "∧") + "&options=" + options,
                    async: true
                });
            } catch (error) {
                $(".calc-solve").html("<hr><span class=error-text>Ошибка: " + error + "</span>");
                $.ajax({
                    url: "https://programforyou.ru/statistics/getStat.shtml?" + "file=calculators//boolean_functions.txt" + "&clicks=" + $("#stat-value").text() + "&function=" + text.replace(/⊕/g, "^").replace(/→/g, "->") + "&error=" + error,
                    async: true
                });
            }
            $(".calc-result").show();
